...
def main():
    try:
        ...
        world.apply_settings(carla.WorldSettings(synchronous_mode=True, fixed_delta_seconds=1.0 / 30.0))
        ...
        batch = [
            carla.command.SpawnActor(blueprints_vehicles[0], ego_transform).then(
        ...
        for i in range(3):
            batch.append(
                carla.command.SpawnActor(blueprints_vehicles[i + 1], other_vehicles_transforms[i]).then(
                    carla.command.SetAutopilot(carla.command.FutureActor, True)
                )
            )
        ...
        while True:
            ...
            velocity_str = (
                "{:.2f}, ".format(ego_velocity.x) + "{:.2f}".format(ego_velocity.y) + ", {:.2f}".format(ego_velocity.z)
            )
