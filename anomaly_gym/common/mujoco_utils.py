"""Source: https://github.com/Farama-Foundation/Gymnasium-Robotics/blob/main/gymnasium_robotics/utils/mujoco_utils.py."""
...
def deg2rad(x: float):
    """Compute angle in radians from degree."""
    return linear_map(x, 0, 180.0, 0, np.pi)


def map_to_mpi_pi(x: float):
    """Map angle x to (minus)-pi, pi."""
    return ((x + np.pi) % (2 * np.pi)) - np.pi


def map_to_0_2pi(x: float):
    """Map angle x to 0, 2pi."""
    return x % (2 * np.pi)


def get_mocap_pos(model: MjModel, data: MjData, name: str):
    """Get the position of a mocap body by name."""
...
def get_mocap_quat(model: MjModel, data: MjData, name: str):
    """Get the quaternion of a mocap body by name."""
    body_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, name)
    assert body_id != -1, f"Body with name '{name}' is not part of the model!"
    mocap_id = model.body_mocapid[body_id]
...
def get_site_xpos(model, data, name):
    ...
    if site_id == -1:
        site_names, _, _ = extract_mj_names(model, mujoco.mjtObj.mjOBJ_SITE)
        raise ValueError(f"Site with name '{name}' is not part of the model!, choose one of {site_names}")
...
def extract_mj_names(
...
) -> tuple[tuple[str, ...] | tuple[()], dict[str, int], dict[int, str]]:
    if obj_type == mujoco.mjtObj.mjOBJ_BODY:
        ...
        raise ValueError(
            f"`{obj_type}` was passed as the MuJoCo model object type. The MuJoCo model object type can only be of the following mjtObj enum types: {MJ_OBJ_TYPES}."
        )
